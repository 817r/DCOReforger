...
{
	protected const float MAX_TIME_TO_UNCON_HIGH_PRIORITY_S = 10; // Time in seconds to losing consciousness below which we treat healing ourselves as high priority
	...
	override float CustomEvaluate()
	{
		...
		if (m_AIInfo && m_AIInfo.GetBleedTimeToUnconscious() < MAX_TIME_TO_UNCON_HIGH_PRIORITY_S)
			return PRIORITY_BEHAVIOR_HEAL_HIGH_PRIORITY + 6;
