...
{	
	...
	override float CustomEvaluate()
	{
		...
		if (baseTarget == m_CombatComponent.GetCurrentTarget())
		{
			...
			float targetScore = m_CombatComponent.m_WeaponTargetSelector.CalculateTargetScore(baseTarget);
			
			if (baseTarget.IsEndangering() || targetScore >= SCR_AICombatComponent.TARGET_SCORE_HIGH_PRIORITY_ATTACK)
				scoreTarget = PRIORITY_BEHAVIOR_ATTACK_HIGH_PRIORITY;
			else if (m_bSelected)
				scoreTarget = PRIORITY_BEHAVIOR_ATTACK_SELECTED;
			else
				scoreTarget = PRIORITY_BEHAVIOR_ATTACK_NOT_SELECTED;
			return scoreTarget;
		}		
