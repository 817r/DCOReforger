modded class SCR_AISuppressGroupClusterBehavior : SCR_AISuppressBehavior
{
	protected const int VOLUME_UPDATE_INTERVAL_MS = 1*500;
	protected const float FIRE_RATE_SCALING_MAX_DISTANCE = 1000; // Max distance for fire rate scaling
	
	protected const float THREAT_MAX_INCREASE = 0.3; // Max increase of group threat potential per tick
	protected const float THREAT_MAX_DECREASE = 0.10; // Max decrease of group threat per tick while in decrease period
	protected const float THREAT_POTENTIAL_DECAY = 0.05; // How much per tick group threat potential is decaying while in decay period
	...
	protected const int THREAT_MAX_PEAK_REACTION_DURATION_MS = 60*1000; // How long we should react on reaching peak (intensification of fire rate while in peak threat)
	...
	{	
		...
		// Remove clamp!
		return Math.Clamp(fireRate, 0.05, 3);
	}
