class DCO_AICombatMoveLogic_SuppressiveDefensive : SCR_AICombatMoveLogicBase
{
	...
	{	
		...
		if (!wp || SCR_EntityWaypoint.Cast(wp))
		{
			...
			movePos = DefendPoint;
			float defRad = SCR_DCO_AIGroupConfigComponent.GetDCOGroupAIConfigComponent(myGroup).m_DefendRadius;
			float myDistToWp = vector.DistanceXZ(DefendPoint, m_MyEntity.GetOrigin());
			bool tgtInWaypoint = vector.DistanceXZ(DefendPoint, targetPos) < defRad;
			avoidStraightPathDir = GetAvoidStraightPathDir(); // Use flanking
			
			if (myDistToWp > defRad)
			{
