...
class AOSpawnClass: GenericEntityClass
{

};

class AOSpawn: GenericEntity
{
	[Attribute("50", UIWidgets.EditBox, "Size of sphere")]
	float m_size; 	
	...
	override private void EOnInit(IEntity owner)
	{
		...
		autoptr PhysicsGeomDef geoms[] = {PhysicsGeomDef("", PhysicsGeom.CreateSphere(m_size), "material/default", EPhysicsLayerDefs.Vehicle | EPhysicsLayerDefs.Character)};
	...
	override void _WB_AfterWorldUpdate(float timeSlice)
	{
		...
		dbgShape = Shape.CreateSphere(color, ShapeFlags.TRANSP | ShapeFlags.DOUBLESIDE | ShapeFlags.NOZWRITE | ShapeFlags.ONCE | ShapeFlags.NOOUTLINE, GetOrigin(), m_size);
