...
class DCO_ObjectiveAreaClass : SCR_CaptureAreaClass
{
}

//! Callback delegate used by events regarding faction ownership in SCR_CaptureArea.
void CaptureAreaTickEventDelegate(DCO_ObjectiveArea area, Faction currentOwner, int score);
...
*/
class DCO_ObjectiveArea : SCR_CaptureArea
{
	[Attribute("", UIWidgets.EditBox, "Area Name", category: "CaptureAndHold")]
	...
	[Attribute("5", UIWidgets.Slider, "Duration in seconds before each point is awarded.", params: "0.1 100.0 0.1", category: "CaptureAndHold")]
	protected float m_fTickRate;

	[Attribute("5", UIWidgets.Slider, "Point awarded per tick.", params: "1 1000 1", category: "CaptureAndHold")]
	protected int m_iScorePerTick;

	[Attribute("0", UIWidgets.Slider, "Point awarded per tick when contested.", params: "0 1000 1", category: "CaptureAndHold")]
	...
	*/
	protected override Faction EvaluateOwnerFaction()
	{
		...
		if (!owningFaction)
		{
			...
			DCO_War_GameMode manager = DCO_War_GameMode.GetAreaManager();
