...
class DCO_FortifyBehavior : SCR_AIBehaviorBase
{
	ref SCR_BTParamAssignable<vector> m_Target = new SCR_BTParamAssignable<vector>("RetreatToPoint");
	ref SCR_BTParamAssignable<vector> m_LookAt = new SCR_BTParamAssignable<vector>("RetreatFrom");
	ref SCR_BTParamRef<BaseTarget> m_RetreatFromTarget = new SCR_BTParamRef<BaseTarget>("RetreatTargetFrom");
	
	void DCO_FortifyBehavior(SCR_AIUtilityComponent utility, SCR_AIActivityBase groupActivity, vector retreatFromTargetPos, BaseTarget retreatFromTarget, float priorityLevel = PRIORITY_LEVEL_NORMAL)
	{
		vector tempVector;
		m_Target.Init(this, retreatFromTargetPos);
		m_LookAt.Init(this, tempVector);
...
class DCO_FortifyActivity : SCR_AIActivityBase
{
	void DCO_FortifyActivity(SCR_AIGroupUtilityComponent utility, AIWaypoint relatedWaypoint, float priorityLevel = PRIORITY_LEVEL_NORMAL)
	{
		SetPriority(PRIORITY_ACTIVITY_FORTIFY);
