class DCO_DefendActivityTactics : SCR_AIActivityBase
{
	...
	//-------------------------------------------------------------------------------------------------
	void SCR_AIDefendActivity(SCR_AIGroupUtilityComponent utility, vector defendDirection, float priority = PRIORITY_ACTIVITY_DEFEND, float priorityLevel = PRIORITY_LEVEL_NORMAL)
	{
		InitParameters(defendDirection, priorityLevel);
		m_sBehaviorTree = "AI/BehaviorTrees/Chimera/Group/ActivityDefend.bt";
		SetPriority(priority);
		leaderEntitiy = utility.GetOwner().GetParentGroup().GetLeaderEntity();
		float radius = 50;
		vector waypointPositionWorld = utility.GetOwner().GetParentGroup().GetOrigin();

		if (defendDirection == vector.Zero) // use orientation of the waypoint
		{
			m_vDefendDirection.m_Value = GetDefendDirection(leaderEntitiy, radius);
		}
		else
		{
			m_vDefendDirection.m_Value =  (defendDirection - waypointPositionWorld).Normalized() * radius + waypointPositionWorld;
		}
	}
	...
	{
		vector originOfLocalSpace = leaderEntitiy.GetOrigin();	
	...
	{
		SCR_AIMessage_Defend defendMessage = SCR_AIMessage_Defend.Create(directionAxis, angleRange, false, m_fPriorityLevel.m_Value, m_RelatedWaypoint, this);
